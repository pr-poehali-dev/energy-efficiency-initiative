...
type FireTabKey = typeof FIRE_LOAD_TABS[number]["key"]
...
export function ReferenceSection({ sectionRef }: { sectionRef?: (el: HTMLElement | null) => void } = {}) {
  ...
  const [mode, setMode] = useState<"simple" | "fireload">("simple")
    ...
    >
      <div className="mx-auto flex w-full max-w-7xl flex-col">
        ...
        >
          ...
          <p className="font-mono text-xs text-foreground/60 md:text-sm">/ Нормативные значения для расчётов</p>
        </div>

        {/* Mode switcher */}
        <div
          className={`mb-4 flex gap-2 flex-wrap transition-all duration-700 ${isVisible ? "opacity-100" : "opacity-0"}`}
        ...
        {mode === "simple" && (
          <div
            className={`flex flex-col flex-1 min-h-0 transition-all duration-700 ${isVisible ? "opacity-100" : "opacity-0"}`}
            style={{ transitionDelay: "150ms" }}
          >
            <div className="mb-4 flex gap-2 flex-wrap">
              {SIMPLE_REFS.map((cat, i) => (
                <button
                  key={cat.category}
                  onClick={() => setActiveSimple(i)}
        ...
        {mode === "fireload" && (
          ...
          >
            ...
            <div className="flex-1 overflow-auto rounded-xl border border-foreground/10 bg-foreground/5 backdrop-blur-sm">
                ...
                <table className="w-full text-xs md:text-sm">
                  <thead className="sticky top-0 bg-background/80 backdrop-blur-sm">
                    <tr className="border-b border-foreground/10">
                      ...
                      <th className="px-3 py-3 text-right font-mono text-xs text-foreground/40 uppercase tracking-widest">α×10⁻³</th>
                ...
                <table className="w-full text-xs md:text-sm">
                  <thead className="sticky top-0 bg-background/80 backdrop-blur-sm">
                    <tr className="border-b border-foreground/10">
                      ...
                      <th className="px-3 py-3 text-right font-mono text-xs text-foreground/40 uppercase tracking-widest">Скорость выгор., кг/(м²·с)</th>
                      <th className="px-3 py-3 text-right font-mono text-xs text-foreground/40 uppercase tracking-widest">Пламя, м/с</th>
                      <th className="px-3 py-3 text-right font-mono text-xs text-foreground/40 uppercase tracking-widest">О₂, кг/кг</th>
                      <th className="px-3 py-3 text-right font-mono text-xs text-foreground/40 uppercase tracking-widest">Токс., кг/кг</th>
                    </tr>
                  </thead>
                  <tbody>
